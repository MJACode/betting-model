...
WNBA injuries are ingested daily (7am pipeline) from the ESPN hidden API, the same source as MLB/NHL. `injury_ingestor.run_injury_ingestor` now defaults to `["MLB", "NHL", "WNBA"]`. Rows land in the shared `injuries` table (`sport='WNBA'`); the WNBA feature engine already consumes them as `home/away_injury_adj` + `home/away_has_returnee`. The game-model gate additionally vetoes a BET when a star (top-2 by last-10 minutes) is Out with `status_ts` ≤ the quote — not as an XGB column (`docs/game_injury_gate.md`).
