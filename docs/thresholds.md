...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-19 PAPER steam rule.** Fade consensus OVER steam (tix ≥75 and money ≥ tix), bet UNDER, max 2 per slate. `RULE=blunt` restores the old ≥70 ticket-cut; `RULE=top1` is the searched t70-keep-one alternate. Juice floor `MIN_UNDER_PRICE` unset (−115 failed holdout). INSERT gated by `MLB_TOTAL_PUBLIC_FADE_PUBLISH` default 0. Stored edge is (tickets−50)/100. `mlb_over_under` stays paused. `docs/mlb_total_public_fade.md`. |
...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-19 PAPER steam rule.** Fade public OVER steam (tix ≥75 and money ≥ tix), max 2 / slate, bet UNDER. `RULE=top1` is the searched t70-keep-one alternate. INSERT off until `MLB_TOTAL_PUBLIC_FADE_PUBLISH=1`. `docs/mlb_total_public_fade.md`. |
