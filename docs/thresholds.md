...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Fade consensus OVER, bet UNDER at best DK/FD/MGM/WH open. INSERT gated by `MLB_TOTAL_PUBLIC_FADE_PUBLISH` default 0. **I24 target:** tickets ≥80 ∩ open under American **[-110, −100]** inclusive (`JUICE_MIN`/`JUICE_MAX`; not a ≥ −115 floor); optional `MAX_PER_SLATE=2`. Measured n=38 +9.51u +25% (Jun/Jul/Sep all green). Code `TICKET_PCT` stays 70. `mlb_over_under` stays paused. `docs/mlb_total_public_fade.md`. |
...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Fade public OVER, bet UNDER. INSERT off until `MLB_TOTAL_PUBLIC_FADE_PUBLISH=1`. I24: t80 ∩ under **[-110, −100]**. `docs/mlb_total_public_fade.md`. |
