...
| `mlb_runline` | 68% | 11% | 2026-07-02 CORRECTION #2: the 2026-06-28 loosen to 0.55/0.10 ("48-41 +14.9% plateau") was computed on a sign bug in `v_model_full_outcome_record` (away picks graded with +home_spread instead of −home_spread — flips every one-run game). Corrected (validated 30/31 vs settlements): 0.55/0.10 = 35-56 **-20.6%**; every prob floor <0.68 negative at volume. Corrected optimum **0.68/0.11 = 19 bets 13-6 +20.0%** (pocket 0.68-0.70 × 0.09-0.12 all +6..+20%; 9 away +1.5 / 10 away -1.5). Small sample. 2026-07-04: model swapped to v20260704_121650 (2019-2024+2026, holdout 2025, CalErr 2.95%); cut carried over UNVALIDATED (2025 has no RL prices, 2026 now in-sample; in-sample check 5-0 all away +1.5). Expect ~1-2 picks/month. **2026-08-21 DORMANT; 2026-08-31 PAUSED (mike).** Honest era −6.93%. 2026-09-15: `mlb_spread_market` exists at 1.8pp but INSERT is gated (`MLB_SPREAD_MARKET_PUBLISH` default 0). GROK Pin-vs-DK ≥2pp ~−5%. Not an unpause. `docs/mlb_runline_ou_edge_search.md`. |
| `mlb_spread_market` | 0% | 1.8% | **2026-09-15 PAPER-GATED rule.** De-vig Pinnacle, bet a bettable soft book at the same ±1.5. 328 bets through Aug 2026 **+4.16%**, early +1.53%/210, late +8.84%/118 on that construction. GROK Pin-vs-DK ≥2pp ~−5% n≈200 — do not live-INSERT until `MLB_SPREAD_MARKET_PUBLISH=1`. `docs/mlb_runline_ou_edge_search.md`. |
| `mlb_total_market` | 0% | 2.0% | **2026-09-15 PAPER-GATED GROK rule.** Pin OPEN no-vig lean vs DK OPEN juiced implied, equal total, DK-only, 2pp. GROK Apr–Jul ~+11% n≈103. Not bettable-soft de-vig (May–Jun 2pp −11.13%/79) and not best-soft implied. INSERT gated by `MLB_TOTAL_MARKET_PUBLISH` default 0; flip only after the worker GROK job confirms. `mlb_over_under` stays paused. Do not resurrect 0.50/0.06. `docs/mlb_runline_ou_edge_search.md`. |
...
| `mlb_runline` | 68% | 11% | 2026-07-02 CORRECTION #2: the 06-28 0.55/0.10 loosen rested on the view sign bug (corrected: -20.6%/91). New optimum 0.68/0.11 = 19 bets 13-6 +20.0%. 2026-07-04: model swapped to v20260704_121650, cut carried over unvalidated (very low expected volume). **2026-08-21 DORMANT; 2026-08-31 PAUSED (mike).** 2026-09-15 `mlb_spread_market` at 1.8pp is paper-gated (`MLB_SPREAD_MARKET_PUBLISH` default 0). GROK Pin-vs-DK ≥2pp ~−5%. Not an unpause. `docs/mlb_runline_ou_edge_search.md`. |
| `mlb_spread_market` | 0% | 1.8% | **2026-09-15 PAPER-GATED rule.** Pinnacle vs bettable soft, equal ±1.5. 328 bets through Aug +4.16% on that construction; GROK ≥2pp Pin-vs-DK ~−5%. INSERT off until env=1. `docs/mlb_runline_ou_edge_search.md`. |
| `mlb_total_market` | 0% | 2.0% | **2026-09-15 PAPER-GATED GROK rule.** Pin lean vs DK implied, 2pp, DK-only. GROK ≥2pp Apr–Jul ~+11% n≈103. INSERT off until `MLB_TOTAL_MARKET_PUBLISH=1` after the worker GROK job. `docs/mlb_runline_ou_edge_search.md`. |
