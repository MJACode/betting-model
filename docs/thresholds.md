...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Default still fade OVER tickets ≥70, bet UNDER at best DK/FD/MGM/WH open. 2026-09-19 grid: t70 is a coverage artifact (−11.5% on complete slates; 12/12 VOID that day). Recommended paper env is **t90 / juice any / max 3 per slate**. INSERT gated by `MLB_TOTAL_PUBLIC_FADE_PUBLISH` default 0. Do not flip. `mlb_over_under` stays paused. `docs/mlb_total_public_fade_selective.md`. |
...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Default still ≥70 tickets. Recommended paper env t90 / any juice / max 3. INSERT off. `docs/mlb_total_public_fade_selective.md`. |
