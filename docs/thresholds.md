...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Fade consensus OVER tickets ≥70 (env `MLB_TOTAL_PUBLIC_FADE_TICKET_PCT`; 80 supported), bet UNDER at best DK/FD/MGM/WH open (fallback DK). INSERT gated by `MLB_TOTAL_PUBLIC_FADE_PUBLISH` default 0. Top-K is mandatory: `MAX_PER_SLATE` clamped 1–2, default `2` + `RANK=ticket` (2026-09-19 holdout: t70 top-2 +11.2% / 48 vs all-pass +5.9% / 101; t75 ∩ −115 top-1 is June-only +19.4% / 13, pooled +5.7% / 24; gap/juice/EV-floor failed). Ticket% is the cut; stored edge is (tickets−50)/100, not a calibrated probability. `mlb_over_under` stays paused. `docs/mlb_total_public_fade.md`. |
...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Fade public OVER ≥70 tickets (env; 80 supported), bet UNDER. INSERT off until `MLB_TOTAL_PUBLIC_FADE_PUBLISH=1`. Top-K `MAX_PER_SLATE` clamped 1–2 (default 2 / `RANK=ticket`). `docs/mlb_total_public_fade.md`. |
