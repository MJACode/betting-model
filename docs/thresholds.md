...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Fade consensus OVER tickets, bet UNDER at best DK/FD/MGM/WH open (fallback DK). INSERT gated by `MLB_TOTAL_PUBLIC_FADE_PUBLISH` default 0. Code `TICKET_PCT` default 70; recommended env **80** + `MAX_PER_SLATE=2` + `RANK=ticket` (t80 top-2 +17.8% / 42; t70 top-2 +11.2% / 48). Cap clamped 1–2. t75 ∩ −115 top-1 is June-only. t90 / cap-3 is #751 paper-only, not this path. `mlb_over_under` stays paused. `docs/mlb_total_public_fade.md`. |
...
| `mlb_total_public_fade` | 0% | 0% | **2026-09-16 PAPER rule.** Fade public OVER, bet UNDER. INSERT off until `MLB_TOTAL_PUBLIC_FADE_PUBLISH=1`. Code `TICKET_PCT` 70; recommended env **80** + `MAX_PER_SLATE=2` + `RANK=ticket`. `docs/mlb_total_public_fade.md`. |
