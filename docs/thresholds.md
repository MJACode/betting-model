...
| `mlb_total_market` | 0% | 2.0% | **2026-09-15 PAPER-GATED Pin-vs-soft-devig logger.** Same construction as `mlb_spread_market`, equal total, 2pp. Measured May–Jun **−11.13%/79**. INSERT gated by `MLB_TOTAL_MARKET_PUBLISH` default 0 — do **not** flip that env because GROK Pin-lean vs DK implied (~+11% n≈103) is a different construction. `mlb_over_under` stays paused. Do not resurrect 0.50/0.06. `docs/mlb_runline_ou_edge_search.md`. |
...
| `mlb_total_market` | 0% | 2.0% | **2026-09-15 PAPER-GATED Pin-vs-soft-devig logger.** 2pp equal total, measured −11.13%/79. INSERT off. GROK +11% is a different construction — not this env. `docs/mlb_runline_ou_edge_search.md`. |
