...
| Date | File | Entry |
|---|---|---|
| 2026-09-12 | [2026-09](./2026-09.md) | 2026-09-12, session 299 - a reply turned the opener's "BUF +1" into "Bills -1" by reading scored_line (the HOME number) as the Bills' line, and it was published. The data was right (picks_log INSERT, Discord posts the label; 4,543 BET rows, 0 flagged). tracking/pick_integrity now refuses and does not ledger any pick whose label disagrees with its side and line on all six send paths, pick_label_integrity goes CRIT, the app's four raw renders use formatSideLine (the fourth, All books, found by the UX review after the scan passed), and CLAUDE.md 00 says QUOTE A PICK FROM ITS LABEL |
