...
| Date | File | Entry |
|---|---|---|
| 2026-09-13 | [2026-09](./2026-09.md) | 2026-09-13, session 302 - denser PickCard follow-up on #703: density analogs (feel only; ASCII 140–160pt wins over Kraken ~9 rows), Designer rec that Live never shows the lock as the lone big American (Now — + Locked caption), GameStatusPill stays the only clock, no LineQuoteRow |
| 2026-09-13 | [2026-09](./2026-09.md) | 2026-09-13, session 301 - denser PickCard: Edge is the hero number, Model/EV/stake collapse to one caption, one book CTA, BET badge Today-only. Live Now vs Locked (in-play DK snapshot, lock is caption). Slip and movement gate on decisionOdds so a non-DK prop can Add to slip. medSoft token. |
