...
- **`docs/mobile_picks_prompt.md`/17 mobile SQL** — added WNBA model thresholds to all three SQL filter blocks: `wnba_moneyline` (prob-only, ≥66%), all 5 WNBA prop models (≥60% prob / ≥8% edge, placeholder — tune after 50+ live picks).
...
- **Still TODO (Phases 4-5):** overnight `nba_api` backfill 2019–2025 → train 8 models (trainer is sport-agnostic; Poisson branch exists) → scorer `run_scorer` WNBA branch + `run_wnba_prop_scorer` → backtest + threshold tuning → `paper_tracker` settlement from `wnba_player_game_log` → `docs/mobile_picks_prompt.md`/17 mobile SQL.
...
- **Still TODO (Phases 3-5):** `features/wnba_feature_engine.py` + `features/wnba_prop_feature_engine.py`, overnight `nba_api` backfill 2019–2025, train 8 models, scorer `run_scorer` WNBA branch + `run_wnba_prop_scorer`, backtest + threshold tuning, `paper_tracker` settlement from `wnba_player_game_log`, `docs/mobile_picks_prompt.md`/17 mobile SQL.
...
- **Display:** Streamlit dashboard "Today's Picks" cards now show "👥 Public on this side — bets: X% | money: Y%" when present. Claude mobile picks prompt (`docs/mobile_picks_prompt.md`) SELECT adds `public_bet_pct`/`public_money_pct` and a new "Public" column ("63% bets / 71% money", "—" when absent).
...
- `docs/mobile_picks_prompt.md` (daily workflow) rewritten for the new schedule. `docs/thresholds.md` Signal Flip Rule updated from "5 daily runs" to "16 hourly runs". Mobile chart prompt F5 borderline-flag wording updated since F5 odds are now refreshed every hour (no longer locked to 11am snapshot).
...
- `dashboard/app.py` filter SQL builder: skips the edge clause for any model in `PROB_ONLY_MODELS`. Mobile picks SQL (`docs/mobile_picks_prompt.md`) and evaluation SQL (`docs/thresholds.md`) updated in CLAUDE.md to do the same. HR clause is now `model_id = 'mlb_prop_batter_hr' AND model_probability >= 0.20` — no edge filter.
...
- Files changed: `config.py`, `models/scorer.py`, `dashboard/app.py`, `CLAUDE.md` (`docs/sports/mlb.md` HR note, `docs/mobile_picks_prompt.md` mobile SQL + prompt, `docs/thresholds.md` BET signal table + ACTION_THRESHOLDS table + filtered picks SQL).
...
- All `docs/mobile_picks_prompt.md` and 17 SQL filters updated to reflect 62%/7% F5 ML thresholds.
...
- Updated config.py, `docs/mobile_picks_prompt.md`, and `docs/thresholds.md` to reflect disabled F5 O/U/RL and new prop pitcher K thresholds. ACTION_THRESHOLDS and picks filter SQL updated throughout.
...
- Raised all F5 thresholds to 65% prob / 15% edge (was 60%/10%, 57%/7%, 58%/8%). Updated config.py, `docs/mobile_picks_prompt.md` query, `docs/thresholds.md` threshold tables.
...
- Planned full MLB player props infrastructure (Phase 2). Decisions locked in `docs/sports/mlb.md`.
...
- Diagnosed why F5 ML picks were never appearing. Three bugs found and fixed:
  ...
  3. **Mobile picks query**: hardcoded SQL in Claude mobile project didn't include `mlb_f5_moneyline`.
     Updated in CLAUDE.md and `docs/thresholds.md` query — must also update the Claude mobile Project instructions manually.
