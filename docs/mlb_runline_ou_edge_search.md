...
| Run line | **Do not live-publish pure Pin-vs-soft spreads.** GROK Pin-open vs DK-open ≥2pp: ~**−5% n≈200**. Earlier same-day bettable-soft de-vig at **1.8pp** was +4.16% / 328 through Aug (both halves +) — a different construction, and GROK’s ≥2pp result is the one that says “more work.” Code for `mlb_spread_market` at 1.8pp remains; INSERT is gated off. `mlb_runline` stays paused. **2026-09-19 hunt** (`docs/mlb_runline_edge_hunt.md`): public fade / money-heavy / home-dog / RLM / steam / public×steam are dead as publishers. Ticket top-2 **+1.2% / 42** (Sep red) and money-heavy top-3 (K-peak vs top-2/top-4) are not shipped. Hunt is **deliverable B**; next market is `mlb_over_under`. |
