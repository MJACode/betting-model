...
def step_push_notifications(run_date: str, dry_run: bool = False) -> bool:
    """
    Push a summary of new / dropped signals to opted-in devices, and post newly
    locked BET signals to their sport's Discord channel. Must run LAST — after
    opening-signal capture — so it sees this refresh's locked signals.
    Idempotent (push_sent ledger); never re-notifies or re-posts a signal.
    """
    try:
