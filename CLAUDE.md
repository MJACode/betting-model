...
because every §17 threshold was swept on DK-implied edge and best-of-N pricing runs **~2pp
...
- **Data source — nflverse weekly player stats (free, no key):** GitHub release assets on nflverse-data, tag `stats_player`, asset `stats_player_week_{season}.csv` (~19K rows/season; one row per player per game with passing/rushing/receiving/defense splits; REG+POST). Verified live: 2024 + 2025 both 200; an unpublished season 404s. The legacy `player_stats` tag carries old asset names ≤2024 — do NOT use it. `game_date` isn't in the weekly file — joined from the nflverse games.csv schedule (`NFLVERSE_GAMES_URL`, the host §28 already reaches from the worker). Config: `NFLVERSE_PLAYER_STATS_URL_TMPL` + `NFL_PLAYER_STATS_BACKFILL_SEASONS` (default 3).
...
- **Live BET record to date: 7-11, −$546 flat** (18 settled, 1 open; runline 0-3, totals 5-6, win prob 2-2). Small sample — §21 says treat the first 50 live picks as the calibration set, so thresholds were deliberately left alone.
...
**Session summary (2026-08-30, session 139 — settled LIVE bets were invisible in every record; now folded into their sport's totals):**
- Matt: "The record tab is not showing the live bets that settled for NCAAF, not sure if that's a similar issue across other sports." **It is not NCAAF-specific — every live bet ever settled was invisible**: all four track-record views carry `p.is_live IS NOT TRUE`, so 92 settled live BETs (83 MLB + 9 NCAAF) never reached the Record tab. NCAAF was simply the first sport where the gap was noticeable, because NCAAF live is ALL it has settled so far (its pre-game `ncaaf_spread` row shows 0 picks), so the sport read as completely empty. Branch `claude/record-tab-live-bets-ncaaf-jlnk45`.
- **The exclusion was not a mistake, and this is the trap: `picks.is_live` carries TWO populations.** (1) real in-play picks from the live scorers — 325 rows across `mlb_live_*` / `ncaaf_live_*`; (2) the **session-114 repair rows — 14,113 PRE-GAME prop picks** retroactively flagged `is_live` because they were scored against in-play prices after first pitch. A naive "drop the filter" would have resurrected 396 settled contaminated prop bets into the honest record — the exact thing session 114 repaired. So the filter could only be *narrowed*, never removed.
- **`model_id LIKE '%\_live\_%'` separates them exactly, verified against every distinct model_id in `picks`**: all 5 live models match, none of the 17 repaired prop models do, and **zero non-is_live models match**, so the OR clause is purely additive on is_live rows and can never widen the pre-game set.
- **Decision (asked): fold live into the sport totals** — Matt chose it over a separate "Live" line or leaving Record pre-game only. So a live bet is now just a bet, per §1c.
- **Migration `track_record_include_live_models` (applied)** patches `v_public_track_record` + `_daily` via `pg_get_viewdef` + replace (the runline sign-fix technique — the giant grading CASEs are never re-transcribed), and is **idempotent**: it checks for the pattern marker and skips rather than double-wrapping (re-ran it live and the numbers were unchanged). Live models land in each view's `other` CTE — they sit outside the MLB/WNBA full-outcome whitelist — which is where this actually takes effect; the occurrences in `clv` / `dates_fo` / `fo_base` are no-ops kept identical so there is one predicate to reason about. `v_model_full_outcome_record` / `_picks` and `mv_scored_pick_outcomes` are deliberately **untouched** (their whitelist already excludes live models, so it would be a no-op there).
- **Measured effect: 513 → 547 picks, 295-195-23 → 314-210-23, +$0.71 on +$3,400 staked (overall ROI 10.79% → 10.11%).** NCAAF 0 → 9 (3-6, −$391); MLB 425 → 450 (+`mlb_live_total_runs` 16-9, +$392). `v_public_track_record` and `_daily` still reconcile to the penny (547 / $5,417.10 / $53,600 both), and `security_invoker` + anon SELECT were preserved.
- **Two more "totals" surfaces had the same gap, both fixed** so all three publish one population: the **daily recap** (`dailyResults.ts` dropped every `is_live` row; `fetchDayPicks` also filtered server-side — that filter is removed and the split now lives only in `computeDailyResults`, one tested definition instead of two that can drift; safe because the busiest day on record is ~3.2k rows *including* every is_live row against the 5k cap, and NONE rows sort last) and the **Discord recap** (`_SETTLED_SQL`). Verified the exact post-interpolation SQL against production: it picks up precisely the 34 live bets and zero repair rows.
- **NEW `thresholds.isLiveModel` / `isContaminatedPregamePick`** are the app-side mirror of the SQL predicate, so the phone and the DB can never disagree about what counts. `_SETTLED_SQL` became a **raw** string: `'%%\_live\_%%'` in a normal string is an invalid Python escape and would be a SyntaxWarning on Matt's 3.14 (it compiles clean on the sandbox's 3.11, so this would only have shown up on his machine).
- **Verification:** full pytest **1,169 passed / 0 failed** against an `origin/master` worktree baseline of 1,168 passed + 1 failed (the documented worktree-path assertion in `test_nfl_opener`) — zero regressions. 2 new Python tests + 6 new `verify_retired_models` checks + 6 new `verify_daily_results` checks, and **all were mutation-tested**: reverting the SQL fails exactly the 2 new Python tests, reverting the client guard fails 7 daily-recap checks. `npx tsc --noEmit` = 28 errors, **error set byte-identical to master** (stash/worktree-compared). 26 of 27 verify scripts pass; `verify_units` fails identically on master (pre-existing stale fixtures, unrelated).
- **Flagged, NOT fixed (pre-existing, unreachable on the happy path):** `computeBuiltInModelStats` reads `fetchSettledPicks`, whose `SETTLED_PICK_COLUMNS` omits `is_live`, so if the `v_model_full_outcome_record` fetch ever FAILS the Models tab's fallback would count repair rows for MLB/WNBA props. Fixing it needs an extra column plus a settled-pick cache-version bump; the view path takes precedence normally, so it only bites on a network error.
- The DB change is **live now** (the Record tab picks it up on next refresh, no rebuild); the mobile half ships via the automatic `mobile-ota.yml` push trigger on merge, and the Discord recap on the next worker redeploy.
