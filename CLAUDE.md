...
- **NOT done / deferred:** P3 (Kalshi/prediction-markets eval, live/micro-betting as paid hooks); the public *website* Track Record page (Lovable — views are ready); enforced loss-limits (needs working bet sync); a v2 HR rework. `model_action_thresholds` is a new sync point — update it alongside `config.py`/`thresholds.ts` when thresholds change.
