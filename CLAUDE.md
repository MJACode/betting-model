...
- **Known limitation (pre-existing, not introduced):** the custom-model backtest reads `fetchSettledPicks`, capped at 5,000 rows; there are 3,149 settled picks since paper start, so the cap will bite eventually and silently truncate older history. Daily volume (~500–1,500) is well under the today-board cap. JS-only → ships via the **Mobile OTA update (production)** workflow after merge.
