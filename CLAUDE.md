...
**Scheduling — triggered by Supabase pg_cron, not GitHub's cron (2026-06-20):** GitHub Actions `schedule:` triggers have no timing SLA and were firing the hourly refresh 15–60 min late (or dropping it). Both workflows are now triggered on time by **Supabase pg_cron → GitHub `workflow_dispatch`** (a dispatched run starts within seconds, bypassing GitHub's congested schedule queue). pg_cron job `refresh-picks-hourly` (`0 15-23,0-3 * * *` UTC) dispatches `refresh_picks.yml`; `daily-pipeline-7am` (`0 11 * * *` UTC) dispatches `daily_pipeline.yml`. The trigger SQL lives in `data/migrations/20260620_pg_cron_github_dispatch.sql` (function `public.trigger_github_workflow`, log table `public.cron_trigger_log`); the GitHub PAT is stored in Supabase **Vault** as `github_pat`. The GitHub native `schedule:` crons are **kept as a backup** (a late GitHub run after pg_cron already fired is a harmless duplicate — scorer delete+re-inserts). Full setup + verify + rollback steps in `docs/scheduling_pg_cron.md`. Growth path (Phase 2, not built): move compute to a Render Cron Job + Background Worker for the future live-betting loop.
...
**Session summary (2026-06-20, session 58 — reliable scheduling: Supabase pg_cron → GitHub workflow_dispatch):**
- Matt: "The GitHub refresh picks does not run enough... I asked for every hour but it's constantly delayed. How can we migrate to a more consistent solution." Root cause: GitHub Actions `schedule:` (cron) has no timing SLA — the hourly `refresh_picks.yml` was firing 15–60 min late or being dropped under load (the `:17`-past-the-hour offset only partly helped). It's a *triggering* problem, not a "can't run in the cloud" problem (each refresh is ~2–3 min of Python; only `nba_api` is IP-blocked and already runs on Matt's local Task Scheduler). Branch `claude/github-refresh-consistency-yjy6b7`. Goal: "consistent and to last for growth" → phased plan.
- **Phase 1 (built this session): Supabase pg_cron → GitHub `workflow_dispatch`.** A dispatched run starts within seconds, bypassing GitHub's congested schedule queue. **Zero pipeline-code changes** — both workflows already declare `workflow_dispatch:`. New `data/migrations/20260620_pg_cron_github_dispatch.sql`: enables `pg_cron`+`pg_net`; `public.trigger_github_workflow(text)` (security definer, reads the PAT from Vault, `net.http_post` to the dispatches endpoint, graceful SKIPPED no-op if the secret is missing); `public.cron_trigger_log` (RLS on, no anon policy); two cron jobs — `refresh-picks-hourly` (`0 15-23,0-3 * * *` UTC = 11am–11pm ET, 13×) and `daily-pipeline-7am` (`0 11 * * *` UTC = 7am ET). GitHub native `schedule:` crons kept as backup (a late GitHub run after pg_cron is a harmless duplicate — scorer delete+re-inserts; spends a few extra Odds API credits, thin later once proven). Setup/verify/rollback guide in `docs/scheduling_pg_cron.md`. CLAUDE.md Section 16 daily-workflow updated.
- **Two manual steps remain for Matt (cannot be automated — secret + MCP approvals weren't grantable in this remote session):** (1) create a fine-grained GitHub PAT for `MJACode/betting-model` (Actions=R/W, Contents=R); (2) apply the migration SQL (Supabase SQL Editor or MCP `apply_migration`) and run `select vault.create_secret('<PAT>', 'github_pat', ...)` once. The moment the secret exists, on-time triggering starts. Verify: `select public.trigger_github_workflow('refresh_picks.yml');` → a run appears in Actions within seconds; `net._http_response` status 204.
- **Phase 2 (documented, not built): Render Cron Job + Background Worker.** When live in-play betting arrives (a 15s always-on loop GitHub Actions can't host), move `run_pipeline.py` compute to Render — same platform hosts the scheduled refresh and the live worker, removes the GitHub Actions monthly-minutes ceiling, and retires GitHub scheduling (GH Actions stays for CI/mobile builds/training only). Plan file: `~/.claude/plans/i-need-a-plan-silly-marble.md`.
