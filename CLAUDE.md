...
- **Verification:** `_opener_rule` exercised for real (source extracted, dependencies stubbed — the sandbox has no numpy) across all 9 paths incl. the dev-sign in both directions; `_days_until` across absent/garbage/naive/past. New `tests/test_ncaaf_board_visibility.py` (18 tests) — the headline one is generic: **every model in `config.MODELS` must have a `FEATURE_MAP` entry**, which is the whole bug class, not just this instance. `tests/test_ncaaf_cross_book_opener.py` repointed at the helper and its two `return []`-counting assertions rewritten for the new semantics (a decline returns the neutral 0.5 + a reason; it must never hand back the artifact's validated win rate). `npx tsc --noEmit` = **28 errors, byte-identical to master** (stash-compared). **Matt: run `python -m pytest -q tests/` before merging** — no pytest in the sandbox (§7).
