...
- **Limitation surfaced but NOT fixed (Matt deferred it):** only `BET` (3,031) and `AVOID` (124) picks ever settle — **zero dead-zone rows**. So a custom model can only ever *tighten* the built-in cuts: setting prob ≥ 55% on a model whose server cut is 72% backtests to nothing, and the new "No signal" filter always grades zero. `v_model_full_outcome_picks` doesn't help — it's pre-filtered to the built-in models' current thresholds (1,074 rows / 20 models) and lacks the filter columns. A true fix is a server-side view grading every scored pick, which would be a 5th copy of the grading CASE that CLAUDE.md already flags as a maintenance trap.
- **Verification (final):** `verify_custom_model_filters.ts` **69/69** (56 filter + 13 cache: window ≥ settle window, floor at paper start, in-window replacement, deletion handling, sort order). `npx tsc --noEmit` = **27 errors, all the `queries.ts` cast baseline** (one baseline error actually resolved by the new `as unknown as`), **0 in touched files**. All 19 slim columns verified present on `picks`. JS-only → ships via the **Mobile OTA update (production)** workflow after merge.
