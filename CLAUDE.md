...
WHERE signal_type = 'BET'
  AND (
    ...
    OR (model_id = 'wnba_prop_player_assists'    AND model_probability >= 0.54 AND edge >= 0.02 AND (dk_odds IS NULL OR dk_odds >= -140))
    OR (model_id = 'wnba_prop_market'              AND edge >= 0.05 AND (dk_odds IS NULL OR dk_odds >= -140))  -- Pinnacle de-vig market rule; prob floor 0 by design
...
2. Query the picks table joined to games, game_weather, and the latest live DK odds. Use today's date in America/New_York (ET) — never UTC.
   ...
   WHERE p.game_date = '{today_et}'
     ...
     AND (
       ...
       -- wnba_prop_player_points PAUSED 2026-07-11 (was 0.58/0.17)
       -- wnba_prop_player_rebounds PAUSED 2026-07-29 (was 0.69/0.08) — -13.9%/54; every sweep cell negative
       OR (p.model_id = 'wnba_prop_player_assists'    AND p.model_probability >= 0.54 AND p.edge >= 0.02 AND (p.dk_odds IS NULL OR p.dk_odds >= -140))
       OR (p.model_id = 'wnba_prop_market'              AND p.edge >= 0.05 AND (p.dk_odds IS NULL OR p.dk_odds >= -140))  -- Pinnacle de-vig market rule; prob floor 0 by design
       -- wnba_prop_player_threes PAUSED 2026-07-11 (was 0.64/0.12)
       -- wnba_prop_player_pra PAUSED 2026-07-11 (was 0.67/0.16)
...
| `wnba_prop_player_assists` | 54% | 2% | 2026-08-31 NB-head re-cut (nb_r=13.56, 2025 OOS): the old 0.69/0.08 graded -21.4% on the leak-free season and went 3-8 live in Aug; plateau centre 0.54/0.02 = 290 bets 54.1% vs 52.7% BE +3.39%, 8/8 neighbours, Jun AND Aug positive |
| `wnba_prop_market` | 0% | 5% | NEW 2026-08-31 — the NFL market-relative rule ported (de-vig Pinnacle, bet the soft outlier; models/wnba_prop_market). Pinnacle quotes points/rebounds/assists for WNBA. 5pp PRE-COMMITTED from the NFL derivation; -140 floor; insert-once lock; PAPER-FIRST (kill: no positive blind month at >=50 flags) |
...
**Session summary (2026-08-31, session 131 — WNBA verdicts: model-first path CLOSED by experiment; assists NB-head re-cut; the NFL market-relative rule ported to WNBA props):**
- Matt: "retrain WNBA models" → goal sharpened to "a model with ROI above the vig and positive units long term" → "check all prop bets, even paused ones — we can use these models in the playoffs" → approved the assists re-cut + the market-rule port. Branch `claude/wnba-market-rule-and-assists-recut`.
- **THE PRE-REGISTERED POINTS REBUILD STOPPED — both availability modes.** Built `features/wnba_availability.py` (rotation rank, teammates-out vacated workload, usage from the never-read `fg_att/fg3_att/ft_att` columns) + a minutes model (beat naive last-5: holdout MAE 4.40 vs 4.62, season-wise OOF so no leakage) + an NB dispersion head (OOF residual var/mean **3.2x** — every WNBA prop reads P(over) off a raw Poisson that overstates both tails), trained 2019-2025 / holdout 2026, graded at real PRE-TIP DK prices (9,654 post-tip snapshots excluded). Result: 52-54% win vs 53.5-54.3% breakevens; best cells +0.1..+1.2% with plateau ≤2/8 — the documented lone-peak noise signature. **Box mode grades as if we knew perfectly who plays tonight and still has no edge → DK's WNBA points line is efficient for us; per the pre-commitment, STOP, no re-cut.** Harness: `scripts/wnba_points_rebuild.py`. Found en route: `is_starter` is 100% NULL 2019-2025 (and in ALL ~105K NBA rows — the same dead role feature sits in 9 live NBA prop models), so `_recent_starter()` emits a constant 0 for every historical row; availability must come from box-score presence, NOT `injuries` (WNBA rows only from 2026-06-07 — NULL for 100% of train, dropna would delete the matrix).
- **ALL FIVE props re-swept on leak-free 2026 lines (`scripts/wnba_prop_sweep.py`), each under BOTH heads (poisson = production, nbinom = method-of-moments r):** points STOP (its lone +0.53% cell self-flagged 3/4 months negative), threes STOP (4-15 at its own cut), pra STOP, rebounds best cell +0.03% = breakeven (stays paused; NB-head trend improving Jul/Aug — re-check post-playoffs). **The never-run `wnba_line_sweep` finally ran:** `wnba_spread` every cell negative on clean lines (definitively paused); `wnba_over_under` only pocket is 30 over-only bets +9.1% that trips the sweep's own one-directional warning, +0.9% at volume — stays paused.
- **`wnba_prop_player_assists` re-cut 0.69/0.08 → 0.54/0.02 + NB head (nb_r=13.56, genuine 2025 OOS).** The old cut graded **-21.4%** over the leak-free season and went 3-8 live in August — the high-conviction tail is exactly where the Poisson head overstates certainty. Plateau centre: **290 bets 54.1% vs 52.7% BE +3.39%, 8/8 neighbours positive, June AND August positive** (poisson-head variant 0.52/0.06 = +3.78%/215, also 8/8). New artifact `wnba_prop_player_assists_20260831_213734.pkl` (means untouched, nb_r stamped via `scripts/wnba_assists_nb_head.py`); `run_wnba_prop_scorer` now routes probs through the NB-aware `_nfl_prop_probs` + `_push_adjusted` (artifacts without nb_r are byte-identical on x.5 lines; integer lines gain correct push mass). **Registry flip SQL is in the script's output — APPLY ONLY AFTER MERGE** (flipping early points the worker at a pkl not on master = the session-51 skip). Old 20260531 assists pkl kept until the flip is verified, then remove.
- **NEW `wnba_prop_market` — the NFL market-relative rule ported (de-vig Pinnacle, bet the soft outlier).** Rationale: with the model-first path closed by experiment, the only edge class validated in this repo is market-vs-market (nfl_prop_market +10.2%/+10.8% blind over 954; NCAAF opener +10.9%/1050) — and **Pinnacle WNBA player props started flowing 2026-08-27** (points/rebounds/assists; it declines threes, which is itself information). `models/wnba_prop_market.py` IMPORTS the NFL module's `find_bets/best_per_prop/devig` (one implementation of de-vig, like-line discipline, dedupe) + a pre-tip quotes loader (parsed-timestamp guard, session-106 discipline); `scripts/wnba_prop_market_card.py` is the plumbing (card → -140 blanket floor → insert-once publish, the opener lock). Settlement rides `_settle_prop_picks` via `_PROP_MARKET_STAT_BY_MODEL` (the FROM_PROP_MARKET sentinel generalized from NFL-only); player_id resolved from the game log or the flag is SKIPPED LOUDLY (never an unsettleable pick). Wired: daily step after wnba-prop-scoring + `--step wnba-prop-market` + `refresh_pass.sh`. Config: min_prob **0.0** (model_probability IS Pinnacle's de-vigged number; the edge is the whole signal), min_edge **0.05 PRE-COMMITTED from the NFL derivation** (6pp chosen greedily went negative blind — not to be chased), -140 floor, deliberately NOT in PROP_MODELS (no artifact to train/health-check). **PAPER-FIRST; kill: no positive blind month at >=50 flags → close, never re-threshold.**
- **Live dry-run on real accrued days:** 8/28 two flags (+7.4pp, +5.2pp), 8/29 zero flags (105 like-line comparisons — clean), 8/30 one flag, 8/31 no-slate no-op. ~1 flag/day expected. The two retro-gradable flags both LOST (n=2 — recorded, not narrated away).
- **Verification:** 43 new tests (20 availability incl. a look-ahead invariance test, 8 engine-context incl. v1 backward-compat byte-identity, 15 market-rule incl. selector-vs-settler map tripwires and insert-once) — full suite **1038 passed / 2 failed, both pre-existing** (`test_system_health_checks` signal-delivery pair, stash-verified on master). Card validated against production data via a temporary scoped read-only DB role (created/dropped through the Supabase MCP — the local `.env` DATABASE_URL password is STALE/rotated and every local DB run is broken until Matt updates it).
- **Matt's runbook:** (1) update `.env` DATABASE_URL password; (2) merge before the next 6am run (threshold_sync from master would otherwise revert the assists cut + the table rows); (3) after merge, apply the registry SQL printed by `python -m scripts.wnba_assists_nb_head` (or ask Claude — it's in the session log) then remove the superseded assists pkl; (4) **re-paste §16 into the Claude-mobile project instructions** (assists line now 0.54/0.02 + new wnba_prop_market OR-line); (5) `eas update` not needed — mobile change is thresholds/meta fallback only, server table drives display.
- Also this session: `scripts/wnba_prop_sweep.py` + `scripts/wnba_points_rebuild.py` are reusable harnesses (the NBA port will want both); design docs published as artifacts "Above the Vig" (verdicts) and "Sharp Anchor" (full pipeline design, three-edge-class thesis, build order).
