...
- **NOT done / deferred:** the rest of P3 (live/micro-betting as paid hooks); the public *website* Track Record page itself (Lovable build — spec now in `docs/`); a v2 HR rework; freemium paywall infra. `model_action_thresholds` is a sync point — update it alongside `config.py`/`thresholds.ts` when thresholds change.
