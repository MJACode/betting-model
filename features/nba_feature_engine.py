"""
nba_feature_engine.py — NBA game feature builder (moneyline / totals / spread).

Mirrors wnba_feature_engine.py 1:1 (same basketball metrics), reading from the
NBA tables instead:
  • build_nba_game_features      — live scoring (per-game DB lookups)
  • build_bulk_nba_lookups       — bulk-load tables for training/backtest
  • build_nba_features_from_bulk  — fast in-memory ASOF lookups

Feature lists (NBA_H2H_FEATURES / NBA_TOTALS_FEATURES / NBA_SPREAD_FEATURES) and
FEATURE_MAP entries live in feature_engine.py alongside MLB/NHL/WNBA. The unified
dispatcher build_features_for_game() and build_training_dataset() call into here.

Data sources:
  • nba_team_stats        — ASOF season-to-date team metrics (off/def rating, pace,
                            eFG%, FG3%, FT%, reb/ast/TOV%, ppg, pts allowed, W/L,
                            home/away pts, point differential)
  • games (sport='NBA')   — rolling points + rest-days / back-to-back signal
  • injuries (sport='NBA') — shared injury-adjustment helpers
"""
...
def _assemble_nba_features(game_id: str, game_date: str,
    ...
    is_early = int(_gp(home_stats) < NBA_MIN_GAMES or _gp(away_stats) < NBA_MIN_GAMES)
...
def _get_nba_team_stats(conn: DBConnection, team: str, season: int,
                        as_of_date: str) -> dict:
    cols = [
    ...
    sel = ", ".join(cols)
    row = conn.execute(f"""
        SELECT {sel} FROM nba_team_stats
...
def build_nba_game_features(conn: DBConnection, game_id: str, game_date: str,
                            home_team: str, away_team: str, season: int,
                            odds_row: dict = None) -> dict:
    """Build the full feature row for one NBA game (live scoring path)."""
    from data.ingestors.injury_ingestor import query_injuries_for_game

    home_stats = _get_nba_team_stats(conn, home_team, season, game_date)
    away_stats = _get_nba_team_stats(conn, away_team, season, game_date)

    injuries = query_injuries_for_game(conn, "NBA", home_team, away_team, game_date)
    home_inj = injuries["home_injuries"]
    away_inj = injuries["away_injuries"]

    return _assemble_nba_features(
...
def build_bulk_nba_lookups(conn: DBConnection, seasons: list[int]) -> dict:
    """Bulk-load NBA tables for fast in-memory ASOF lookups (training/backtest)."""
    ...
    logger.debug(
        f"NBA bulk loads: {len(ts_rows)} team-stat rows, {len(hist_rows)} hist games, "
...
def _blk_nba_team_stats(bulk: dict, team: str, season: int, game_date: str) -> dict:
    ts = bulk['team_stats']
...
def _blk_nba_rolling_points(bulk: dict, team: str, game_date: str,
                            window: int) -> float | None:
...
def _blk_nba_rest(bulk: dict, team: str, game_date: str) -> int | None:
    if team not in bulk['play_dates']:
        return None
    return _rest_days(bulk['play_dates'][team], game_date)


def _blk_nba_injuries(bulk: dict, team: str, game_date: str) -> list:
...
def build_nba_features_from_bulk(bulk: dict, game_id: str, game_date: str,
                                 home_team: str, away_team: str, season: int,
                                 odds_row: dict | None) -> dict:
    """Build an NBA feature row from pre-loaded bulk data (training/backtest)."""
    return _assemble_nba_features(
        game_id, game_date, home_team, away_team, season,
        _blk_nba_team_stats(bulk, home_team, season, game_date),
        _blk_nba_team_stats(bulk, away_team, season, game_date),
        _blk_nba_rolling_points(bulk, home_team, game_date, 3),
        _blk_nba_rolling_points(bulk, home_team, game_date, 5),
        _blk_nba_rolling_points(bulk, away_team, game_date, 3),
        _blk_nba_rolling_points(bulk, away_team, game_date, 5),
        _blk_nba_rest(bulk, home_team, game_date),
        _blk_nba_rest(bulk, away_team, game_date),
        _blk_nba_injuries(bulk, home_team, game_date),
        _blk_nba_injuries(bulk, away_team, game_date),
        odds_row,
    )
