...
def attach_odds(g: pd.DataFrame, regions: str, quota_guard: int) -> tuple[pd.DataFrame, int]:
    ...
    for i, row in g.iterrows():
        ...
        pu = cand[cand.book == PINNACLE]
        if pu.empty:
            continue
        pu = pu.iloc[0]
        po = overs[(overs.home == row.home_team) & (overs.away == row.away_team) &
                   (overs.book == PINNACLE) & (overs.point == pu.point)]
        g.at[i, "pin_total"] = float(pu.point)
        g.at[i, "pin_under_px"] = int(pu.price)
        if len(po):
            g.at[i, "pin_over_px"] = int(po.iloc[0].price)
            g.at[i, "pin_prob"] = round(devig_two_way(int(pu.price), int(po.iloc[0].price))[0], 4)
...
def next_wait(watched: pd.DataFrame, now: pd.Timestamp) -> tuple[int, str]:
    ...
    # Do not sleep past the moment the nearest game crosses into the fast window.
