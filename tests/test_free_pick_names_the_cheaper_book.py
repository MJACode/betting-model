...
def test_the_tweet_is_silent_when_draftkings_is_already_best():
    t = xp.render_free_pick(_pick(best_book="draftkings", best_odds=-102.0),
                            "2026-09-02")
    assert "at DraftKings" in t and t.count("at ") == 1


@pytest.mark.parametrize("worse", [-130.0, -102.0])
def test_the_tweet_never_offers_a_worse_price_as_an_alternative(worse):
    t = xp.render_free_pick(_pick(best_book="betmgm", best_odds=worse),
                            "2026-09-02")
    assert "BetMGM" not in t


def test_the_clause_is_dropped_not_truncated_when_it_will_not_fit():
    """A half-written price is worse than none. The hashtags earn their place
    (+21% engagement) and the pick itself is the post.

    The label length is chosen so the tweet fits WITHOUT the clause and
    overflows WITH it — that is the only case this branch decides. A longer
    label overflows on its own and hits the pre-existing `[:MAX_TWEET]`
    truncation, which is a different (and already tested) behaviour.
    """
    label = "X" * 200
    base = xp.render_free_pick(_pick(label=label, best_book="draftkings",
                                     best_odds=-102.0), "2026-09-02")
    assert len(base) <= xp.MAX_TWEET, "fixture no longer fits without the clause"

    t = xp.render_free_pick(_pick(label=label), "2026-09-02")
    assert len(t) <= xp.MAX_TWEET
    assert "ESPN BET" not in t, "the clause was added and then truncated"
    assert t.endswith(xp.hashtags_for("MLB")), "the tags were pushed off the end"


def test_an_unpriced_pick_produces_no_clause():
    assert xp._better_price_clause({"dk_odds": None, "best_book": "betmgm",
                                    "best_odds": 100}) is None
    assert xp._better_price_clause({"dk_odds": -110, "best_book": None,
                                    "best_odds": None}) is None
...
def test_the_free_card_names_the_cheaper_book():
    ...
    assert "+100" in value and "ESPN BET" in value
...
def test_every_offerable_book_has_a_human_name_on_both_surfaces():
    ...
    for book in config.BEST_LINE_BOOKMAKERS:
        ...
        if book != config.ODDS_API_BOOKMAKER:
