...
ACTION_THRESHOLDS: dict = {
    ...
    "mlb_live_total_runs": {"min_prob": 0.72, "min_edge": 0.14},  # 2026-09-09 mike: 0.70 -> 0.72 on the 47-slate replay of the v20260908_230751 artifact. 2026-09-10: that replay paired without the stale-quote guard; on fresh quotes the same cut is 21 bets 12-9 (2026) / 92 bets 60.9% (2025) — docs/thresholds.md "The forward check on fresh quotes"
    ...
    # MLB totals Pin-vs-soft-devig logger. PAPER-GATED at 2pp: same
    # construction as spreads, equal total. Measured May–Jun 2pp
    # −11.13%/79 — not a publish candidate. GROK Pin-lean vs DK implied
    # (~+11%) is a different construction; it is not this id and is not
    # a reason to set MLB_TOTAL_MARKET_PUBLISH=1. mlb_over_under stays
    # paused. The find_total_bets wall (MIN_EDGE_TOTALS=1.0) stays; the
    # card passes 0.02 explicitly.
...
PAUSED_MODELS: set = {
    ...
    # 2026-09-15: the *publish path* for this market is mlb_spread_market
    # (1.8pp Pinnacle-vs-bettable-soft), gated by MLB_SPREAD_MARKET_PUBLISH
    # (default 0). This id stays paused until mike unpauses it with
    # Updated-By. Do not resurrect the 2026-09-02 0.50/0.06 RE-CUT.
    ...
    # mlb_total_market at 2pp (Pin-vs-soft-devig, measured −11%), log-only
    # until MLB_TOTAL_MARKET_PUBLISH=1 — do not flip that env on GROK.
    "mlb_over_under",
    ...
    #   mlb_total_market  — MLB totals Pin-vs-soft-devig logger (not mlb_over_under)
...
# / the app / push do not see a BET until mike sets the Railway variable
# and redeploys. This PR does not flip either flag. Spreads: GROK
# Pin-vs-DK ≥2pp ~−5% n≈200 — do not INSERT. Totals: Pin-vs-soft-devig
# 2pp is −11.13%/79 — do not INSERT. GROK +11% is a different
# construction; confirming it is not permission to set
# MLB_TOTAL_MARKET_PUBLISH=1 (that env writes Pin-vs-soft).
...
SCORING_METHODS: dict = {
    ...
    "mlb_total_market":    "rule",    # models/mlb_game_market.py — Pin-vs-soft-devig 2pp, INSERT gated; measured −11%
...
MODEL_PROB_THRESHOLDS: dict = {
    ...
    # mlb_live_win_prob + mlb_live_runline RETIRED 2026-08-30 (see LIVE_MODELS).
    "mlb_live_total_runs": 0.72,  # 2026-09-09 mike: 0.70 -> 0.72. Swept on the 520-game / 47-slate replay of the NEW artifact (v20260908_230751). 2026-09-10: that replay paired quotes WITHOUT production's stale-quote guard (#458); on fresh quotes the plateau is gone — 21 bets 12-9 on 2026, 92 bets 60.9% [52-69] on the bought 2025 history. Cut unchanged pending mike's decision (docs/thresholds.md "The forward check on fresh quotes"). There is NO live CLV — docs/live_betting.md has the measurement.
    # NCAAF (FBS) — PLACEHOLDER cuts, deliberately tighter than our other launch
    # defaults. A Saturday slate is ~60-80 FBS games, so a loose cut would fire
    # 30+ picks in one afternoon. Tune from the 2025 holdout sweep (Phase 4),
    # sliced by game_tier (P4 vs G5) and week bucket.
